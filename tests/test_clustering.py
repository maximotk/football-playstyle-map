...
def test_rule_based_cluster_dominant():
    ...
    # row 1 → c1=0.55, c2=0.30, so still c1
    assert result.loc[1, "cluster_dominant"] == "c1"
    # row 2 → both c1 & c2 ~0.40+0.25, should be combo
    assert "+" in result.loc[2, "cluster"]
    # row 3 → no strong cluster, expect "Mixed"
    assert result.loc[3, "cluster"] == "Mixed"
